<?php
// ============================================================
// cases.php - BARANGAY CASES & COMPLAINTS
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add') {
        ...
        // ── SERVER-SIDE: block future date_filed ──
        if ($date_val > date('Y-m-d')) {
            echo json_encode(['ok' => false, 'msg' => 'Date filed cannot be a future date. Cases must be filed on or before today.']);
            $conn->close(); exit;
        }
        ...
        $stmt->bind_param('ssissss',
            $case_number, $date_val, $complainant_id, $respondent_name, $nature, $status, $det_val
        );
    ...
    if ($action === 'edit') {
        ...
        // ── SERVER-SIDE: block future date_filed on edit too ──
        if ($date_val > date('Y-m-d')) {
            echo json_encode(['ok' => false, 'msg' => 'Date filed cannot be a future date.']);
            $conn->close(); exit;
        }
        ...
        $stmt->bind_param('ssissssi',
            $case_number, $date_val, $complainant_id, $respondent_name, $nature, $status, $det_val, $id
        );
    ...
    if ($action === 'generate_case_number') {
        $year    = date('Y');
        $stmt    = $conn->prepare(
        ...
        $next = sprintf('BRY-%s-%03d', $year, $seq + 1);
        echo json_encode(['ok' => true, 'case_number' => $next]);
        $conn->close(); exit;
    }
...
if ($search) {
    $where[]  = "(c.case_number LIKE ? OR r.first_name LIKE ? OR r.last_name LIKE ? OR c.respondent_name LIKE ?)";
    $like     = "%$search%";
...
if ($filter_nature) {
    $where[]  = "c.nature = ?";
    $params[] = $filter_nature;
    $types   .= 's';
}

$whereSql  = $where ? ' WHERE ' . implode(' AND ', $where) : '';

$countStmt = $conn->prepare("SELECT COUNT(*) AS c FROM cases c JOIN residents r ON c.complainant_id = r.id" . $whereSql);
...
$data_stmt   = $conn->prepare(
    "SELECT c.*, r.first_name, r.last_name
     FROM cases c JOIN residents r ON c.complainant_id = r.id"
    . $whereSql .
    " ORDER BY c.created_at DESC LIMIT ? OFFSET ?"
);
$data_stmt->bind_param($fetchTypes, ...$fetchParams);
$data_stmt->execute();
$cases = $data_stmt->get_result();
$data_stmt->close();

$all_residents = $conn->query(
    "SELECT id, first_name, last_name FROM residents WHERE is_active = 1 ORDER BY last_name, first_name"
)->fetch_all(MYSQLI_ASSOC);

$conn->close();

// Today's date for the date cap (passed to JS)
$today = date('Y-m-d');
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar {
            ...
            background: #0a2a5e; display: flex; flex-direction: column;
            border-right: 4px solid #c8a84b; z-index: 100;
        }
        ...
        .nav-label {
            font-size: 10px; color: rgba(255,255,255,0.3); letter-spacing: 0.12em;
            text-transform: uppercase; padding: 0 1.25rem; margin-top: 12px;
        }
        .nav-item {
            display: flex; align-items: center; gap: 10px;
            padding: 10px 1.25rem; color: rgba(255,255,255,0.65);
        ...
        .btn-logout {
            ...
            border-radius: 6px; padding: 8px; color: rgba(255,255,255,0.55);
        ...
        .form-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1rem; padding: 1.5rem; }
        .form-group label { display: block; font-size: 12px; font-weight: 600; color: #2a5a7a; margin-bottom: 4px; }
        .form-group input, .form-group select, .form-group textarea {
            width: 100%; padding: 8px 10px; border: 1px solid #d0dae8;
            border-radius: 8px; font-family: inherit; font-size: 13px;
        }
        .form-group textarea { resize: vertical; min-height: 80px; }

        /* Date field hint */
        .date-hint { font-size: 11px; color: #7a8aaa; margin-top: 3px; display: block; }
        .date-hint i { color: #c8a84b; }

        button, .btn {
            background: #0a2a5e; color: white; border: none;
            padding: 8px 16px; border-radius: 40px; cursor: pointer; font-size: 13px; font-weight: 500;
        }
        ...
        .case-table th, .case-table td { padding: 12px 1rem; text-align: left; border-bottom: 1px solid #eef0f6; font-size: 13px; }
        .case-table th { background: #f7f9fc; font-weight: 600; color: #2a4a6e; }
        ...
        .page-links a:hover { background: #0a2a5e; color: #fff; border-color: #0a2a5e; box-shadow: 0 0 8px rgba(10,42,94,0.4); }
        .page-links span.current { background: #0a2a5e; color: #fff; border-color: #0a2a5e; box-shadow: 0 0 8px rgba(10,42,94,0.6); font-weight: 700; }
        ...
        .modal-content {
            background: white; max-width: 700px; width: 90%;
            border-radius: 16px; max-height: 90vh; overflow-y: auto;
        }
        .modal-header {
            padding: 1rem 1.5rem; border-bottom: 1px solid #ddd; font-weight: bold;
            display: flex; justify-content: space-between;
            background: #0a2a5e; color: #f0d080; border-radius: 16px 16px 0 0;
        }
...
<div class="main">
    <div class="topbar">
        <h1><i class="fas fa-gavel"></i> Cases &amp; Complaints</h1>
    </div>
    <div class="content">

        <!-- Add Case Button -->
        <div style="margin-bottom: 1.5rem;">
            <button id="openAddModalBtn"><i class="fas fa-plus-circle"></i> File New Case</button>
        </div>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:1.5rem; align-items:center;">
            <form method="GET" style="display:flex; gap:8px; flex-wrap:wrap; flex:1;">
                <input type="text" name="search" placeholder="Search case #, complainant, respondent..."
                       value="<?= htmlspecialchars($search) ?>"
                       style="padding:8px 12px; border:1px solid #d0dae8; border-radius:8px; font-size:13px; min-width:240px;">
                <select name="status" style="padding:8px 12px; border:1px solid #d0dae8; border-radius:8px; font-size:13px;">
                ...
                <select name="nature" style="padding:8px 12px; border:1px solid #d0dae8; border-radius:8px; font-size:13px;">
                    <option value="">All Nature</option>
                    <option value="Dispute"          <?= $filter_nature === 'Dispute'          ? 'selected' : '' ?>>Dispute</option>
                    <option value="Noise Complaint"  <?= $filter_nature === 'Noise Complaint'  ? 'selected' : '' ?>>Noise Complaint</option>
                    <option value="Physical Assault" <?= $filter_nature === 'Physical Assault' ? 'selected' : '' ?>>Physical Assault</option>
                    <option value="Property Damage"  <?= $filter_nature === 'Property Damage'  ? 'selected' : '' ?>>Property Damage</option>
                    <option value="Theft"            <?= $filter_nature === 'Theft'            ? 'selected' : '' ?>>Theft</option>
                    <option value="Other"            <?= $filter_nature === 'Other'            ? 'selected' : '' ?>>Other</option>
                </select>
                <button type="submit"><i class="fas fa-search"></i> Filter</button>
                <?php if ($search || $filter_status || $filter_nature): ?>
                    <a href="cases.php" style="background:#ccc; color:#333; padding:8px 16px; border-radius:40px; text-decoration:none; font-size:13px;">
        ...
        <!-- Cases Table -->
        <div class="card">
            <div class="card-header"><i class="fas fa-list"></i> All Cases (<?= $total ?> total)</div>
                        ...
                        <?php if ($cases && $cases->num_rows > 0): ?>
                            ...
                            <tr data-id="<?= $c['id'] ?>">
                                ...
                                <td class="action-buttons">
                                    <button class="editBtn"   data-id="<?= $c['id'] ?>"><i class="fas fa-edit"></i> Edit</button>
                                    <button class="deleteBtn btn-danger" data-id="<?= $c['id'] ?>"><i class="fas fa-trash-alt"></i> Delete</button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="8" style="text-align:center; padding: 2rem;">No cases found. Click "File New Case" to add one.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <?php if ($total_pages > 1):
                $extra = ($search        ? '&search=' . urlencode($search)        : '')
                       . ($filter_status ? '&status=' . urlencode($filter_status) : '')
                       . ($filter_nature ? '&nature=' . urlencode($filter_nature) : '');
            ?>
            <div class="pagination">
                <span>Page <?= $page ?> of <?= $total_pages ?> (<?= $total ?> total)</span>
                <div class="page-links">
                    <?php if ($page > 1): ?>
                        <a href="?page=<?= $page - 1 . $extra ?>" class="page-arrow">&#8592;</a>
                    <?php else: ?>
                    ...
                    <?php for ($p = 1; $p <= $total_pages; $p++):
                        ...
                        else: ?><a href="?page=<?= $p . $extra ?>"><?= $p ?></a><?php
                        endif;
                    endfor; ?>
...
<div id="caseModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <span id="modalTitle"><i class="fas fa-file-alt"></i> File New Case</span>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <div style="display:flex; gap:6px; align-items:center;">
                        <input type="text" id="case_number" required placeholder="Generating…"
                               style="flex:1; background:#f0f3f9; font-weight:600; color:#0a2a5e; letter-spacing:0.04em;"
                               readonly>
                        <button type="button" id="regenCaseNumBtn" title="Generate a new case number"
                                style="padding:8px 10px; border-radius:8px; flex-shrink:0; font-size:13px;">
                            <i class="fas fa-sync-alt"></i>
                        </button>
                    </div>
                    <small style="color:#7a8aaa; font-size:11px; margin-top:3px; display:block;">
                        Auto-generated. Click <i class="fas fa-sync-alt"></i> to refresh.
                    </small>
                </div>
                <div class="form-group">
                    <label>Date Filed *</label>
                    <!-- max is set dynamically by JS to today — future dates are not allowed -->
                    <input type="date" id="date_filed" required>
                    <small class="date-hint"><i class="fas fa-info-circle"></i> Must be today or a past date. Future dates are not accepted.</small>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="respondent_name" required placeholder="Full name of respondent">
                </div>
                <div class="form-group">
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    <label>Details</label>
                    <textarea id="details" rows="3" placeholder="Additional information about the case..."></textarea>
                </div>
            </div>
            <div style="padding: 1rem 1.5rem 1.5rem; text-align: right; display: flex; gap: 8px; justify-content: flex-end;">
                <button type="button" id="cancelModalBtn" style="background:#888;"><i class="fas fa-times"></i> Cancel</button>
                <button type="submit" id="saveBtn"><i class="fas fa-save"></i> Save Case</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    // Today's date from PHP (server time) — used as the max for date_filed
    const TODAY = '<?= $today ?>';
    ...
    // ── Set the date cap on the date_filed field ──
    function capDateFiled() {
        const field = document.getElementById('date_filed');
        field.max = TODAY;   // blocks future dates in the date-picker UI
    }

    // ── Client-side validation before submit ──
    function validateDateFiled() {
        const val = document.getElementById('date_filed').value;
        if (!val) return true; // required check handles empty
        if (val > TODAY) {
            showToast('Date filed cannot be a future date. Cases must be filed on or before today.', 'err');
    ...
    // ── Generate next case number from server ──
    async function generateCaseNumber() {
        const input = document.getElementById('case_number');
        const btn   = document.getElementById('regenCaseNumBtn');
        input.value = '';
        input.placeholder = 'Generating…';
        if (btn) btn.disabled = true;
        try {
            const fd = new FormData();
            fd.append('action', 'generate_case_number');
            ...
            if (data.ok) {
                ...
                input.placeholder = 'Error — enter manually';
    ...
    document.getElementById('openAddModalBtn').onclick = async () => {
        document.getElementById('modalTitle').innerHTML = '<i class="fas fa-file-alt"></i> File New Case';
        document.getElementById('caseForm').reset();
        document.getElementById('caseId').value = '';
        capDateFiled();
        document.getElementById('date_filed').value = TODAY; // default to today
        // Lock case number field and auto-generate
        document.getElementById('case_number').setAttribute('readonly', true);
        document.getElementById('case_number').style.background = '#f0f3f9';
        document.getElementById('regenCaseNumBtn').style.display = '';
        document.getElementById('caseModal').style.display = 'flex';
        await generateCaseNumber();
    };

    // ── Regen button ──
    document.getElementById('regenCaseNumBtn').addEventListener('click', generateCaseNumber);
    ...
    document.querySelectorAll('.editBtn').forEach(btn => {
        btn.onclick = async () => {
            ...
            if (result.ok && result.data) {
                ...
                // In edit mode: allow manual correction, hide regen button
                document.getElementById('case_number').removeAttribute('readonly');
                document.getElementById('case_number').style.background = '';
                document.getElementById('regenCaseNumBtn').style.display = 'none';
                // Cap date_filed so edits can't push it into the future
                capDateFiled();
