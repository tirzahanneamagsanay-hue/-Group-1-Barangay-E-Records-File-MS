...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'add') {
        ...
        // ── SERVER-SIDE: block future incident dates ──
        if ($incident_date > date('Y-m-d')) {
            echo json_encode(['ok' => false, 'msg' => 'Incident date cannot be a future date. Please enter the actual date the incident occurred.']);
            $conn->close(); exit;
        }
        ...
        $stmt = $conn->prepare(
            "INSERT INTO blotter (blotter_number, incident_type, complainant_id, complainant_name, respondent_name,
                                  incident_date, incident_time, location, description, status,
                                  investigating_officer, created_by)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'reported', ?, ?)"
        );
    ...
    if ($action === 'delete') {
        ...
        echo json_encode(['ok' => $ok, 'msg' => $ok ? 'Blotter deleted successfully.' : 'Delete failed: ' . $stmt->error]);
        $stmt->close(); $conn->close(); exit;
    }
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-links span.current { background: #0a2a5e; color: #f0d080; border-color: #0a2a5e; box-shadow: 0 0 8px rgba(10,42,94,0.6); font-weight: 700; }
        ...
        .modal-content {
            background: white; max-width: 650px; width: 90%;
        ...
        .form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; padding: 1.5rem; }
        ...
        <div class="card">
                ...
                <table>
                    ...
                    <tbody>
                        <?php if ($blotters && $blotters->num_rows > 0): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="8" style="text-align:center; padding: 2rem;">No blotter reports found. Click "File Blotter Report" to add one.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="pagination">
                ...
                <div class="page-links">
                    ...
                    if ($start > 1): ?><a href="?page=1<?= $extra ?>">1</a><?php
                        if ($start > 2): ?><span>&hellip;</span><?php endif;
                    endif;
                    for ($p = $start; $p <= $end; $p++):
                        if ($p == $page): ?><span class="current"><?= $p ?></span><?php
                        else: ?><a href="?page=<?= $p . $extra ?>"><?= $p ?></a><?php endif;
                    endfor;
                    if ($end < $total_pages):
                        if ($end < $total_pages - 1): ?><span>&hellip;</span><?php endif;
                        ?><a href="?page=<?= $total_pages . $extra ?>"><?= $total_pages ?></a><?php
                    endif; ?>
        ...
        <form id="blotterForm">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" id="complainant_name" required placeholder="Full name of complainant">
                </div>
                <div class="form-group">
                ...
                <div class="form-group" style="grid-column: 1 / -1;">
                    ...
                    <textarea id="description" rows="3" required placeholder="Detailed account of the incident..."></textarea>
                </div>
                <div class="form-group">
                    <label>Investigating Officer</label>
                    <input type="text" id="investigating_officer" placeholder="Barangay Tanod / Officer assigned">
                </div>
                <!-- Edit-only fields -->
                ...
                <div class="form-group" id="resolutionGroup" style="display:none;">
                    ...
                    <textarea id="resolution" rows="2" placeholder="How was this resolved?"></textarea>
                </div>
            </div>
...
<script>
    // Today's date from PHP (server time) — used as the max for incident_date
    const TODAY = '<?= $today ?>';
    ...
    // ── Set the date cap on the incident_date field ──
    function capIncidentDate() {
        const field = document.getElementById('incident_date');
        field.max = TODAY;                 // blocks future dates in the date-picker UI
    }

    // ── Client-side validation before submit ──
    function validateIncidentDate() {
        const val = document.getElementById('incident_date').value;
        if (!val) return true; // required check handles empty
        if (val > TODAY) {
            showToast('Incident date cannot be a future date. Please enter the actual date the incident occurred.', 'err');
    ...
    document.getElementById('blotterForm').addEventListener('submit', async (e) => {
        ...
        // Only validate date when adding a new report (edit doesn't change incident_date)
        if (!id && !validateIncidentDate()) return;
    ...
    document.getElementById('openAddModalBtn').onclick = () => {
        ...
        capIncidentDate();
        document.getElementById('incident_date').value = TODAY; // default to today
    ...
    document.querySelectorAll('.editBtn').forEach(btn => {
        btn.onclick = async () => {
            ...
            if (result.ok && result.data) {
                ...
                // Keep original incident_date; also cap so edits can't push it to future
                capIncidentDate();
    ...
    document.querySelectorAll('.deleteBtn').forEach(btn => {
        btn.onclick = async () => {
            if (!confirm('Are you sure you want to delete this blotter report? This cannot be undone.')) return;
            const id       = btn.getAttribute('data-id');
            const formData = new FormData();
            formData.append('id', id);
